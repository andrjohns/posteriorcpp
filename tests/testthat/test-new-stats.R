...
# how sd() (and thus bayescpp's existing "sd" column) already treats the
# matrix.
...
expect_matches_reference <- function(x, stat, tolerance = 1e-4) {
  out <- bayescpp::summarise_draws(x, stats = stat)
...
test_that("var matches stats::var(as.vector(m)) and equals sd^2", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("sd", "var"))
...
test_that("rhat_basic differs from rank-normalised rhat on skewed data", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("rhat", "rhat_basic"))
...
test_that("requesting rhat_basic and rhat together matches requesting them separately", {
  ...
  together <- bayescpp::summarise_draws(x, stats = c("rhat", "rhat_basic"))
  rhat_only <- bayescpp::summarise_draws(x, stats = "rhat")
  rb_only <- bayescpp::summarise_draws(x, stats = "rhat_basic")
...
test_that("requesting ess_basic and ess_bulk together matches requesting them separately", {
  ...
  together <- bayescpp::summarise_draws(x, stats = c("ess_bulk", "ess_basic"))
  bulk_only <- bayescpp::summarise_draws(x, stats = "ess_bulk")
  basic_only <- bayescpp::summarise_draws(x, stats = "ess_basic")
...
test_that("all new stats requested together, in reverse order, still match individually", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("ess_basic", "rhat_basic", "var"))
...
test_that("full 12-stat set is in the documented canonical column order", {
  x <- make_draws(300, 4, 4, seed = 9)
  out <- bayescpp::summarise_draws(x, stats = bayescpp:::.bayescpp_all_stats)
  expect_identical(
    names(out),
...
test_that("new stats propagate NA on NaN draws, independently per variable", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("var", "rhat_basic", "ess_basic"))
...
test_that("an unknown stat still errors with the full 12-stat choice list", {
  x <- make_draws(200, 4, 2, seed = 12)
  err <- tryCatch(bayescpp::summarise_draws(x, stats = "bogus"), error = function(e) e)
