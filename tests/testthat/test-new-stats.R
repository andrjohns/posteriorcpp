...
test_that("rhat_basic differs from rank-normalised rhat on skewed data", {
  ...
  x <- make_draws(1000, 4, 5, seed = 5, fun = function(n) {
    stats::rexp(n, rate = 0.5)
  })
...
test_that("requesting rhat_basic and rhat together matches requesting them separately", {
  ...
  together <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = c("rhat", "rhat_basic")
  )
...
test_that("all new stats requested together, in reverse order, still match individually", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = c("ess_basic", "rhat_basic", "var")
  )
...
test_that("full stat set is in the documented canonical column order", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = posteriorcpp:::.posteriorcpp_all_stats
  )
  expect_identical(
    names(out),
    c(
...
test_that("new stats propagate NA on NaN draws, independently per variable", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = c("var", "rhat_basic", "ess_basic")
  )
  expect_true(all(is.na(out[
    out$variable == "...2",
    c("var", "rhat_basic", "ess_basic")
  ])))
  expect_true(all(
    !is.na(out[out$variable == "...1", c("var", "rhat_basic", "ess_basic")])
  ))
  expect_true(all(
    !is.na(out[out$variable == "...3", c("var", "rhat_basic", "ess_basic")])
  ))
})
...
test_that("an unknown stat still errors with the full stat choice list", {
  ...
  err <- tryCatch(
    posteriorcpp::summarise_draws_cpp(x, stats = "bogus"),
    error = function(e) e
  )
