...
expect_matches_reference <- function(x, stat, tolerance = 1e-4) {
  out <- posteriorcpp::summarise_draws_cpp(x, stats = stat)
...
test_that("rhat_basic differs from rank-normalised rhat on skewed data", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = c("rhat", "rhat_basic"))
...
test_that("requesting rhat_basic and rhat together matches requesting them separately", {
  ...
  together <- posteriorcpp::summarise_draws_cpp(x, stats = c("rhat", "rhat_basic"))
  rhat_only <- posteriorcpp::summarise_draws_cpp(x, stats = "rhat")
  rb_only <- posteriorcpp::summarise_draws_cpp(x, stats = "rhat_basic")
...
test_that("requesting ess_basic and ess_bulk together matches requesting them separately", {
  ...
  together <- posteriorcpp::summarise_draws_cpp(x, stats = c("ess_bulk", "ess_basic"))
  bulk_only <- posteriorcpp::summarise_draws_cpp(x, stats = "ess_bulk")
  basic_only <- posteriorcpp::summarise_draws_cpp(x, stats = "ess_basic")
...
test_that("all new stats requested together, in reverse order, still match individually", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = c("ess_basic", "rhat_basic", "var"))
...
test_that("full stat set is in the documented canonical column order", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = posteriorcpp:::.posteriorcpp_all_stats)
  expect_identical(
    names(out),
...
test_that("new stats propagate NA on NaN draws, independently per variable", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = c("var", "rhat_basic", "ess_basic"))
...
test_that("an unknown stat still errors with the full stat choice list", {
  ...
  err <- tryCatch(posteriorcpp::summarise_draws_cpp(x, stats = "bogus"), error = function(e) e)
