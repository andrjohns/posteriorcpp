...
test_that("ess_mean matches posterior::ess_mean() and equals ess_basic exactly", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = c("ess_basic", "ess_mean"))
...
test_that("mcse_mean requested alone matches when sd/ess_basic/ess_mean are also requested", {
  ...
  alone <- posteriorcpp::summarise_draws_cpp(x, stats = "mcse_mean")
  together <- posteriorcpp::summarise_draws_cpp(x, stats = c("mcse_mean", "sd", "ess_basic", "ess_mean"))
...
test_that("mcse_sd requested alone matches when ess_sd is also requested", {
  ...
  alone <- posteriorcpp::summarise_draws_cpp(x, stats = "mcse_sd")
  together <- posteriorcpp::summarise_draws_cpp(x, stats = c("mcse_sd", "ess_sd"))
...
test_that("ess_sd and ess_mean are generally different (distinct estimators)", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = c("ess_mean", "ess_sd"))
...
test_that("new stats propagate NA on NaN draws, independently per variable", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(x, stats = stats4)
