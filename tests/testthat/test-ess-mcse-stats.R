...
test_that("mcse_mean requested alone matches when sd/ess_basic/ess_mean are also requested", {
  ...
  together <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = c("mcse_mean", "sd", "ess_basic", "ess_mean")
  )
...
test_that("all four stats requested together, unordered, still match individually", {
  ...
  out <- posteriorcpp::summarise_draws_cpp(
    x,
    stats = c("mcse_sd", "ess_mean", "mcse_mean", "ess_sd")
  )
  expect_identical(
    names(out),
    c("variable", "ess_mean", "ess_sd", "mcse_mean", "mcse_sd")
  )
