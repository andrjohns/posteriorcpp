...
test_that("stats order in the call does not affect output column order", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("ess_tail", "mean", "rhat", "sd"))
...
test_that("duplicate stats are collapsed to a single column", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("mean", "mean", "sd", "sd", "sd"))
...
test_that("requesting all stats individually reconstructs the default output", {
  ...
  full <- bayescpp::summarise_draws(x)
  piecewise <- lapply(all_stats, function(s) bayescpp::summarise_draws(x, stats = s)[[s]])
...
test_that("an invalid stat name errors with a helpful message", {
  ...
  expect_error(bayescpp::summarise_draws(x, stats = "bogus"))
  expect_error(bayescpp::summarise_draws(x, stats = c("mean", "not_a_stat")))
})

test_that("an empty stats vector errors rather than silently returning nothing", {
  x <- make_draws(200, 4, 2, seed = 7)
  expect_error(bayescpp::summarise_draws(x, stats = character(0)))
})
...
test_that("stats subsetting still handles NaN draws correctly", {
  ...
  out <- bayescpp::summarise_draws(x, stats = c("mean", "rhat"))
...
test_that("shared-work stats (rhat + ess_bulk) still match when requested together or separately", {
  ...
  together <- bayescpp::summarise_draws(x, stats = c("rhat", "ess_bulk"))
  rhat_only <- bayescpp::summarise_draws(x, stats = "rhat")
  ess_only <- bayescpp::summarise_draws(x, stats = "ess_bulk")
